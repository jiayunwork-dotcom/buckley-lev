...
`buckley-lev -http :8080` 启动核算 API，请求体统一为 `{"case": <算例对象>}`（history 另带 `pv` 列表）：

- `GET /health` 健康检查；
- `POST /api/profile` 返回 Welge 切点、激波与无因次剖面点列；
- `POST /api/fractional` 返回分流函数 f(Sw) 采样表；
- `POST /api/sweep` 对 `mu_w/mu_o/sor/swc/krw0/kro0/nw/no` 任一参数做扫描；
- `POST /api/history` 返回突破前后出口/平均饱和度与采收率历史。
