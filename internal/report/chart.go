package report

import (
	"fmt"
	"strings"

	"buckley-lev/internal/welge"
)

// SaturationChart 画出一张 ASCII 饱和度剖面图。
//
// 坐标约定：
//   - 横轴：无因次位置 ξ，从 0（注入端）到 1.25·ξs（原始油藏）；
//   - 纵轴：水饱和度 Sw，从 Swc（底部）到 Sw_inj（顶部）；
//   - '#' 是稀疏波曲线，'|' 是激波（Sw 从 Swf 跳到 Swc），
//     '.' 是激波前方的原始油藏，'@' 是注入端点，'+' 是激波顶部。
//
// 图只用于直观检查“前缘后方是稀疏波而不是斜直线”：稀疏波应表现为
// 一条平滑下凹的曲线，而不是从注入端到前缘的直线。
func SaturationChart(res *welge.Result, width, height int) string {
	if width < 20 || height < 8 {
		return "(图宽/高过小，无法绘制)"
	}
	m := res.Model
	swInj := res.Case.Injection.SwInj
	xiShock := res.Shock.Speed
	xiMax := xiShock * 1.25

	grid := make([][]byte, height)
	for r := range grid {
		grid[r] = bytesOf(width, ' ')
	}

	// 映射函数：ξ→列，Sw→行（Sw 高在上）。
	toX := func(xi float64) int {
		x := xi / xiMax
		if x < 0 {
			x = 0
		}
		if x > 1 {
			x = 1
		}
		return int(x*float64(width-1) + 0.5)
	}
	toY := func(sw float64) int {
		y := (sw - m.Swc) / (swInj - m.Swc)
		if y < 0 {
			y = 0
		}
		if y > 1 {
			y = 1
		}
		return int((1-y)*float64(height-1) + 0.5)
	}
	set := func(x, y int, ch byte) {
		if x >= 0 && x < width && y >= 0 && y < height {
			grid[y][x] = ch
		}
	}

	// 稀疏波曲线：从 ξ=0 到 ξ=ξs 采样。
	for xi := 0.0; xi <= xiShock; xi += xiShock / 200 {
		sw, err := welge.RarefactionSaturation(m, res.Tangent, swInj, xi)
		if err != nil {
			continue
		}
		set(toX(xi), toY(sw), '#')
	}

	// 激波：在 ξ=ξs 处 Sw 从 Swf 垂直落到 Swc。
	xFront := toX(xiShock)
	yUp := toY(res.Tangent.Swf)
	yDown := toY(m.Swc)
	for y := yUp; y <= yDown; y++ {
		set(xFront, y, '|')
	}
	set(xFront, yUp, '+')

	// 注入端点（ξ=0，Sw=Sw_inj）。
	set(toX(0), toY(swInj), '@')

	// 原始油藏（激波前方，ξ>ξs）用 '.' 填充。
	for x := xFront + 1; x < width; x++ {
		set(x, toY(m.Swc), '.')
	}

	// 拼装：左侧固定宽度标签列。
	const leftW = 16
	var b strings.Builder
	b.WriteString("饱和度剖面（纵轴 Sw，横轴 ξ=x/t）\n")
	for r := 0; r < height; r++ {
		frac := 1 - float64(r)/float64(height-1)
		sw := m.Swc + frac*(swInj-m.Swc)
		label := ""
		if r == 0 {
			label = fmt.Sprintf("Sw=%.3f", sw)
		} else if r == height-1 {
			label = fmt.Sprintf("Sw=%.3f", sw)
		}
		b.WriteString(padLeft(label, leftW) + "│" + string(grid[r]) + "\n")
	}
	b.WriteString(strings.Repeat(" ", leftW) + "└" + strings.Repeat("─", width) + "\n")
	b.WriteString(padLeft("ξ=0", leftW) + " " + "ξ=ξs 在激波位置\n")
	return b.String()
}

// padLeft 左对齐到给定宽度。
func padLeft(s string, w int) string {
	if len(s) >= w {
		return s
	}
	return s + strings.Repeat(" ", w-len(s))
}

// bytesOf 返回 n 个 ch 组成的字节切片。
func bytesOf(n int, ch byte) []byte {
	out := make([]byte, n)
	for i := range out {
		out[i] = ch
	}
	return out
}
