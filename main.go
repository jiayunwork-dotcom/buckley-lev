// Command buckley-lev 是一维两相水驱（Buckley–Leverett）核算命令行工具。
//
// 用法：
//
//	buckley-lev profile  <算例.json> [--xi 0,0.3,1]   无因次剖面 + 激波
//	buckley-lev welge    <算例.json>                  Welge 切点与激波运动学
//	buckley-lev fractional <算例.json> [--grid N]     分流函数采样表
//	buckley-lev probe    <算例.json>                  交叉规则实验
//	buckley-lev check    <算例.json>                  只校验输入
//	buckley-lev version                                版本
//	buckley-lev help                                    帮助
//
// 所有非法输入（文件缺失、JSON 错误、物理量越界、切线无解）都写入
// stderr 并返回非零退出码。
package main
...
// cliOptions 是解析后的命令行参数。
type cliOptions struct {
	subcommand string
	file       string
	xi         string // --xi 的值；空表示默认采样
	grid       int    // --grid 的值
	param      string // --param 的值（sweep）
	from, to   float64 // --from / --to 的值（sweep）
	steps      int    // --n 的值（sweep）
}

// parseArgs 手动解析子命令与可选 flag，非法用法返回错误。
...
// renderProfile 组装 profile 子命令的完整输出。
...
// parseFractionsOpt 解析 --xi 选项；未指定时返回 nil（使用默认采样）。
